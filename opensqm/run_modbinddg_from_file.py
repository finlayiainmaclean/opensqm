...
from opensqm.run_cph import ConstantpHRunSettings, SystemState, run_cph

RDLogger.DisableLog("rdApp.warning")

MANIFEST_FILENAME = "modbinddg_manifest.json"

_SOLVENT_RESNAMES = frozenset(("HOH", "WAT", "TIP3", "TIP4", "TIP5", "NA", "CL"))


def _protein_modeller_from_snapshot(snapshot: SystemState) -> Modeller:
    """Return a dry protein Modeller from a CpH lowest-energy snapshot.

    Strips the ligand (any residue whose name starts with ``LIG``) and all
    solvent/ion residues so the result can be passed directly to
    :func:`~opensqm.modbinddg.states.build_bound_state`.
    """
    modeller = Modeller(snapshot.topology, snapshot.positions * unit.nanometers)
    to_delete = [
        r for r in modeller.topology.residues()
        if r.name.startswith("LIG") or r.name in _SOLVENT_RESNAMES
    ]
...
def run_modbinddg(
    ...
    cph_result = cph_result["ph_results"][0] # ph 7
    ...
    if bound_state is None or (need_unbound and unbound_state is None):
        ...
        if bound_state is None:
            ...
            protein_modeller = _protein_modeller_from_snapshot(snapshot)
